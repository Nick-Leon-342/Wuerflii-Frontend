...
export default function PlayerTable({ list_Players, axiosPrivate, tableWidth, lastPlayerAlias, gnadenwurf, setGnadenwurf, showScores, disabled, playerScores, joincode }) {
		...
		<table id={id_playerTable} className='table playerTable' style={{ minWidth: tableWidth, width: tableWidth, maxWidth: tableWidth }}>
			<tbody>
				...
				<tr style={{ display: showScores ? '' : 'none' }}>
					<td style={{ borderLeft: thickBorder, borderRight: thickBorder, borderBottom: `${playerScores ? thickBorder : ''}` }}>Spieler gesamt</td>
					{list_Players?.map((p, i) => {
						return (<td key={i} alias={p.Alias} style={{ borderRight: thickBorder, borderBottom: `${playerScores ? thickBorder : ''}` }}>{playerScores ? playerScores[p.Alias] : 0}</td>)
					})}
				</tr>
				{!disabled && <tr>
